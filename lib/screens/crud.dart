...
class _CrudScreenState extends State<CrudScreen> {
  ...
  Future<void> fetchData() async {
    final response = await http.get(
      Uri.parse('https://binifiy-default-rtdb.asia-southeast1.firebasedatabase.app/binstorage.json'),
    );
    ...
    if (binStorageData.isNotEmpty && index < binStorageData.length) {
      ...
      if (confirmDelete == true) {
        final response = await http.delete(
          Uri.parse('https://binifiy-default-rtdb.asia-southeast1.firebasedatabase.app/binstorage/${index + 1}.json'),
        );
  ...
  if (binStorageData.isNotEmpty && index < binStorageData.length) {
    ...
    if (updatedData != null) {
      String documentId = binStorageData[index]['key']; // Use the actual Firebase-generated key for the document
      final response = await http.patch(
        Uri.parse(
            'https://binifiy-default-rtdb.asia-southeast1.firebasedatabase.app/binstorage/$documentId.json'),
    ...
    return Scaffold(
      ...
      body: Column(
        ...
        children: [
          ...
          ElevatedButton(
            onPressed: () {
              createData(newKeyController.text ,newNameController.text, newDataController.text);
            },
